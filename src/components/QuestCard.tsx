  ...
  if (isSignal) {
    return (
      <Link
        ...
        className="block border border-white/10 bg-[#0f0f0f] rounded-sm p-5 hover:border-[#D85A30]/50 transition-colors group border-l-2 border-l-[#D85A30]/60"
      >
        {/* Status + strength row */}
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          ...
          {quest.signal_strength && (
            <span className="text-xs font-mono px-2 py-0.5 rounded-sm border border-[#D85A30]/30 text-[#D85A30]/70 bg-[#D85A30]/5">
              {quest.signal_strength}
            </span>
          )}
          {quest.market_size && (
            <span className="text-xs font-mono px-2 py-0.5 rounded-sm border border-white/10 text-gray-500">
              {quest.market_size}
            </span>
          )}
        </div>
        ...
        {/* Title */}
        <h3 className="text-[15px] font-semibold text-white leading-snug line-clamp-2 mb-2 group-hover:text-[#D85A30] transition-colors">
          {quest.title}
        </h3>

        {/* Location */}
        {quest.location && (
          <p className="text-xs text-gray-500 font-mono mb-3">📍 {quest.location}</p>
        )}
        ...
        {quest.skills_needed.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {visibleSkills.map((skill) => (
              <span key={skill} className="text-[11px] font-mono px-1.5 py-0.5 border border-white/10 text-gray-400 rounded-sm">{skill}</span>
            ))}
            {extraSkills > 0 && (
              <span className="text-[11px] font-mono px-1.5 py-0.5 border border-white/10 text-gray-400 rounded-sm">+{extraSkills} more</span>
            )}
          </div>
        )}
          ...
          <div className="flex items-center gap-2">
            {quest.poster?.avatar_url ? (
              ...
              <div className="w-5 h-5 rounded-full bg-[#D85A30]/20 flex items-center justify-center">
                <span className="text-[9px] text-[#D85A30] font-bold">{(quest.poster?.display_name ?? '?')[0].toUpperCase()}</span>
              </div>
            )}
            <span className="text-xs text-gray-500">@{quest.poster?.slug ?? 'unknown'}</span>
          </div>
          <span className="text-xs text-[#D85A30]/70 font-mono">🔥 {quest.seen_count ?? 0}</span>
        </div>
      </Link>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className={`text-xs font-mono px-2 py-0.5 rounded-sm ${STATUS_COLORS[quest.status] ?? 'text-gray-400 bg-gray-400/10'}`}>
        ...
        <div className="flex items-center gap-2">
          {quest.poster?.avatar_url ? (
            ...
            <div className="w-5 h-5 rounded-full bg-[#534AB7]/30 flex items-center justify-center">
              <span className="text-[9px] text-[#534AB7] font-bold">{(quest.poster?.display_name ?? '?')[0].toUpperCase()}</span>
            </div>
          )}
          <span className="text-xs text-gray-400">{quest.poster?.display_name ?? 'Unknown'}</span>
        </div>
        <div className="flex items-center gap-3 text-xs text-gray-500 font-mono">
          <span>{quest.claims_count ?? 0}/{quest.max_claimers} claimed</span>
          {quest.deadline && <span>due {formatRelativeTime(quest.deadline)}</span>}
        </div>
      </div>
    </Link>
  );
