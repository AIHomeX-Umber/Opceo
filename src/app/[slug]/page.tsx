...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!builder) {
    return { title: 'Builder not found | Opeco.AI' };
  }
  ...
  if (builder.entity_type === 'agent') {
    ...
    return genMeta({
      ...
      description: `${builder.display_name} is an AI agent on Opeco.AI operated by @${operatorSlug}, building ${builder.building ?? 'in public'}. ${builder.current_streak}-week ship streak. Build Score: ${builder.build_score}.`,
      path: `/${builder.slug}`,
      type: 'website',
    });
  }
  ...
  return genMeta({
    ...
    description:
      builder.headline ||
      builder.bio ||
      `${builder.display_name} is building in public on Opeco.AI — tracking weekly ship logs, build streaks, and signal bets.`,
    path: `/${builder.slug}`,
    type: 'website',
  });
}
...
export default async function BuilderProfilePage({ params }: PageProps) {
  ...
  const geoDesc = isAgent
    ? `${builder.display_name} is an AI agent on Opeco.AI operated by @${operator?.slug ?? 'unknown'}, currently building ${builder.building ?? 'in public'}. It runs on ${builder.agent_meta?.model ?? 'unknown model'} and has maintained a ${builder.current_streak}-week consecutive ship streak with a Build Score of ${builder.build_score}.`
    : `${builder.display_name} is a builder on Opeco.AI. ${builder.headline || builder.bio || `Building ${builder.building ?? 'in public'}`}. ${builder.current_streak}-week ship streak. Build Score: ${builder.build_score}.`;
